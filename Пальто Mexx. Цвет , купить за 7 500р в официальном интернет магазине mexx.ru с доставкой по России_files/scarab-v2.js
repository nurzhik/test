var _scq = _scq || [];
_scq.push(['setMerchantId', '1C01F4661438598A']);
/**package
{
  "name": "scarab-util",
  "version": "2.0.1"
...
var ScarabUtil = function() {
  ...
  var flattenCategories = function(categories) {
    ...
    var flatcats = ScarabArrays.flatmap(categories.split('|'), function(cat) {
      var parts = ScarabArrays.map(trim(cat).split('>'), function(part) {
        return trim(part);
      });
      return ScarabArrays.map(parts, function(p, i) {
        return parts.slice(0, i + 1).join('>');
      });

    });
    var ret = [];
    ScarabArrays.forEach(flatcats, function(fc) {
      if (indexOf(ret, fc) < 0) {
        ret.push(fc);
      }
    });
  ...
  var findCommonCategory = function(products, language) {
    var catField = language ? 'c_category_' + language : 'category';
    var counts = {};

    ScarabArrays.forEach(products, function(product) {
      var cats = flattenCategories(product[catField]);
      ScarabArrays.forEach(cats, function(cat) {
        counts[cat] = (counts[cat] || 0) + 1;
      });
    });

    var best = { score: 0, cat: '' };
    for (var cat in counts) {
      if (counts.hasOwnProperty(cat)) {
        if (counts[cat] >= best.score) {
            if ((best.score === 0 || best.score === counts[cat])
              && cat.length > best.cat.length) {
            best.cat = cat;
          }
          best.score = counts[cat];
        }
      }
    }
    return best.cat;
...
/* jshint ignore:end */
  ...
  var modules = {
    inspector: {
      ...
      devPath: localResource('/inspector/build/scarab-inspector.js')
    },
    discovery: {
  ...
  return {
    ...
    findCommonCategory: findCommonCategory,
    playQueue: playQueue,
    script: {},
    modules: modules,
    loader: loader,
...
var ScarabModule = function(win, config) {
    ...
    Feature.prototype.getDataForRendering = function() {
        ...
        SC.topicLocalized = this.transaction.language ?
            ScarabUtil.findCommonCategory(SC.page.products, this.transaction.language)
            : this.topicLabel;
        SC.recommender = {};
    ...
    var publicInterface = {
        ...
        assistant : function(config) {
            ...
            if (config.forceMobile ||
                    ...
                    && navigator.userAgent.search('Mobi') > -1) {
